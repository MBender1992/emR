...
bt_coxph_meta_analysis <- function(data, time, status, vars, var, meta.group, weights = NULL,
  ...
  res <- pbapply::pblapply(1:length(levels(data[[var]])), function(x) {
    ...
    bt_fit <- pbapply::pbreplicate(n = n.boot, {
      dat_boot <- dat[sample(nrow(dat), size = round(n.sample *
                                                       scaling_factor), replace = T), ]
      if(!is.null(weights)) weights <- dat_boot[[weights]]
      surv <- coxph(as.formula(paste("Surv(", time, ", ", status,
                                     ") ~ ", vars_input, sep = "")), data = dat_boot, weights = weights)
      surv$coefficients
    })
    bt_df <- data.frame(matrixStats::rowQuantiles(bt_fit,
...
# tryCatch(coxph(cox_formula, data = dat_boot), error=function(e) NA)
