...
survplot_eumelareg <-
            ...
            ...)
  {
    ...
    if (!is.null(var)) {
      ...
      if (is.null(plot.margin.left))
        plot.margin.left <- 12 * sqrt(max(nchar(levels)))
    ...
    if (!is.null(var)) {
      fit <-
        surv_fit(
          Surv(eval(parse(text = time)), eval(parse(text = status))) ~ eval(parse(text = var)),
          data = data,
          weights = weights,
          conf.type = conf.type
        )
      ...
      fit <-
        surv_fit(
          Surv(eval(parse(text = time)), eval(parse(text = status))) ~ 1,
          data = data,
          weights = weights,
          conf.type = conf.type
        )
    ...
    if (!is.null(weights)) {
      ...
      pval.ipsw <-
        logrank_IPSW_RISCA(dat_logrank[[time]],
                           dat_logrank[[status]],
    ...
    if (!is.null(var)) {
      ...
      tbl$median <- sapply(1:length(surv_med$median), function(x) {
        paste(
          ...
          " (",
          round(surv_med$lower[x], 2),
          "-",
          round(surv_med$upper[x], 2),
          ")",
          sep = ""
        )
      })
